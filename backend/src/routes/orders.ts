  ...
  app.post("/", async (request, reply) => {
    try {
      ...
      if (body.paymentMethod === "prepaid") {
        ...
        let returnBase = FRONTEND_URL;
        try {
          returnBase = new URL((body as any).returnUrl || FRONTEND_URL).origin;
        } catch {
        ...
        const session = await stripe.checkout.sessions.create({
          ...
          mode: "payment",
          success_url: `${returnBase}/checkout/sukces?session_id={CHECKOUT_SESSION_ID}`,
          cancel_url: `${returnBase}/checkout?stripe_cancel=true`,
